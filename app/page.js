      ...
      <div className="container bg-gray-200 mx-auto w-full px-4 py-4 rounded-lg">
            ...
            {dropdown.map((item) => {
              return (
                <>
                  <div
                    key={item.slug}
                    className="container flex justify-between my-3 px-3 py-3 border-b-2"
                  >
                    <span className="slug">
                      {item.slug} (Available {item.quantity} at ₹{item.price} )
                    </span>
                    <div className="mx-5 flex justify-content items-center">
                      ...
                      <button
                        onClick={() => {
                          deletebuttononpopbox(item.slug);
                        }}
                        disabled={loadingaction}
                        className="delete cursor-pointer inline-block px-3 py-1 mx-3 bg-purple-500 text-white font-semibold rounded-lg shadow-md disabled:bg-purple-200"
                      >
                        {" "}
                        Delete All
                      </button>
        ...
        <div className="container my-8 mx-auto">
          ...
          <table className="table-auto w-full border-black border-2 border-black">
            <thead>
              <tr>
                <th className="px-4 py-2 border border border-gray bg-black text-white">
                  Product Name
                </th>
                <th className="px-4 py-2 border border border-gray bg-black text-white">
                  Quantity
            ...
            <tbody>
              {products.map((product) => (
                <tr className="my-3 mx-3" key={product.slug}>
                  <td className="px-4 py-4 border border-l-3 border-black">
                    {product.slug}
                  </td>
                  <td className=" px-4 py-2 border border-l-3 border-black">
                    {product.quantity}
                  </td>
                  <td className=" px-4 py-2 border border-l-3 border-black">
                    ₹{product.price}
